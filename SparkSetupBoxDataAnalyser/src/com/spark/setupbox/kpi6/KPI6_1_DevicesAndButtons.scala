package com.spark.setupbox.kpi6
...
object KPI6_1_DevicesAndButtons {
  ...
  def main(args: Array[String]) = {
    ...
    .map(line => {
      ...
      if (event_id.length() > 0 && event_id.equals("107")) {
      ...
      val buttonName =((xml \ "nv").filter(node => (node \@ "n") == "ButtonName")) \@ "v" 
      
      (deviceId, buttonName) 
       
    })
    .filter(touple => {touple._1.length() > 0})
    .filter(touple => { touple._2.trim().length() > 0})
    .groupByKey()
    
    println("Devices with their buttons: ")
