package com.spark.setupbox.kpi5
...
object KPI5_1_CountJunkRecords {
  ...
  def main(args: Array[String]) = {
    ...
    .map(line => {
      ...
      if (event_id.length() > 0 && event_id.equals("0")) {
      ...
      var badBlockNum = 0
      val badBlock =((xml \ "nv").filter(node => (node \@ "n") == "BadBlocks")) \@ "v" 
    ...
    println("Total number of junk records")
