...
public class DeptWiseReportJobWhenSalaryIsLowAndNotPromotedReducer extends Reducer<Text, HRResourceWritable, Text, Text> {
	...
	protected void reduce(Text dept, Iterable<HRResourceWritable> hrResources, Context context) throws IOException, InterruptedException {
		...
		for (HRResourceWritable hrResource : hrResources) {
			if(hrResource.getSalaryCategory().toString().equalsIgnoreCase(HRUtil.SAL_LOW) && !hrResource.getHasPromoted().get()) {
				continue;
			}
