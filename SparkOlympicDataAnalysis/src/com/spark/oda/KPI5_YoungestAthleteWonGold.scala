...
object KPI5_YoungestAthleteWonGold {
  ...
  def main(args: Array[String]) = {
    ...
    if (args.length > 1) {
      System.err.print("Usages: KPI5_YoungestAthleteWonGold <inputFilePath>")
      System.exit(1)
    }

    //Creating spark session
    val sparkSession = SparkSession.builder().appName("KPI5_YoungestAthleteWonGold").getOrCreate()
    ...
    //Youngest athlete who won gold in each category of sports in each Olympic
    sparkSession.sql("select athlete, year, sport, min(age) from olympic_data group by athlete, year, sport having sum(countGoldMedals) >= 1").show()
...
//bin/spark-submit --class com.spark.oda.KPI5_YoungestAthleteWonGold /home/barun/SparkWorkshopArtifacts/OlympicData/olympicDataAnalyserJob.jar /home/barun/SparkWorkshopArtifacts/OlympicData/olympic_Data.csv