...
object KPI3_Top10GoldMedalist {
  ...
  def main(args: Array[String]) = {
    ...
    if (args.length > 1) {
      System.err.print("Usages: KPI3_Top10GoldMedalist <inputFilePath>")
      System.exit(1)
    }

    //Creating spark session
    val sparkSession = SparkSession.builder().appName("KPI3_Top10GoldMedalist").getOrCreate()
    ...
    sparkSession.sql("select athlete, sum(countGoldMedals) from olympic_data group by athlete order by sum(countGoldMedals) desc").show(10)
...
//bin/spark-submit --class com.spark.oda.KPI3_Top10GoldMedalist /home/barun/SparkWorkshopArtifacts/OlympicData/olympicDataAnalyserJob.jar /home/barun/SparkWorkshopArtifacts/OlympicData/olympic_Data.csv